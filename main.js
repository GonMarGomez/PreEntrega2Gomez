...
let tablets = [];
notebooks.push(
  new Producto("LENOVO V15", "PENTIUM N5030 4GB 1TB 15.6", "218289")
);
notebooks.push(
  new Producto("HP 245 G8", "AMD ATHLON 3020E 1TB 4GB 14", "218499")
);
notebooks.push(
  new Producto("ASUS X515EA", "I3 115G4 4GB SSD 256GB 15.6 W11", "249990")
);
...
function comprador(seleccion) {
  switch (seleccion) {
    case "notebooks":
      let todasLasNotebooks = notebooks.map(
        (producto) =>
          `
                 --NOMBRE: ${producto.nombre} CARACTERISTICAS: ${producto.caracteristicas} PRECIO: ${producto.precio}--`
      );
      let seleccionNotebook = prompt(
        `Seleccione el producto escribiendo el nombre ${todasLasNotebooks}`
      ).toLocaleUpperCase();
      const NotebookEncontrada = notebooks.find((i) => {
        return i.nombre === seleccionNotebook;
      });
      let productoSeleccionado =
        prompt(`El producto seleccionado es: ${NotebookEncontrada.nombre}-- Precio: ${NotebookEncontrada.precio}.
      ...
      break;
    case "tablets":
      let todasLasTablets = tablets.map(
        (producto) =>
          `
                 --NOMBRE: ${producto.nombre} CARACTERISTICAS: ${producto.caracteristicas} PRECIO: ${producto.precio}--`
      );
      let seleccionTablet = prompt(
        `Seleccione el producto escribiendo el nombre ${todasLasTablets}`
      ).toLocaleUpperCase();
      const tabletEncontrada = tablets.find((i) => {
        return i.nombre === seleccionTablet;
      });
      let tabletSeleccionada =
        prompt(`El producto seleccionado es: ${tabletEncontrada.nombre}-- Precio: ${tabletEncontrada.precio}.
            Desea agregarlo al carrito? SI/NO`).toLocaleLowerCase();
      ...
      break;
    case "celulares":
      let todosLosCelulares = celulares.map(
        (producto) =>
          `
                     --NOMBRE: ${producto.nombre} CARACTERISTICAS: ${producto.caracteristicas} PRECIO: ${producto.precio}--`
      );
      let seleccionDeCelular = prompt(
        `Seleccione el producto escribiendo el nombre ${todosLosCelulares}`
      ).toLocaleUpperCase();
      const celularEncontrado = celulares.find((i) => {
        return i.nombre === seleccionDeCelular;
      });
      let celularSeleccionado =
        prompt(`El producto seleccionado es: ${celularEncontrado.nombre}-- Precio: ${celularEncontrado.precio}.
                Desea agregarlo al carrito? SI/NO`).toLocaleLowerCase();
...
function vendedor() {
  let tipoProducto = prompt(
    "Escriba el tipo de producto: TABLETS, NOTEBOOKS O CELULARES:"
  ).toLocaleLowerCase();
